  ...
  return (
    <div ref={ref} onMouseMove={onMove} onMouseLeave={reset} style={{ width, height: h, perspective: 900, cursor: interactive ? "pointer" : "default", flexShrink: 0 }}>
      <div style={{
        ...
        transformStyle: "preserve-3d", backfaceVisibility: "hidden", WebkitBackfaceVisibility: "hidden", background: r.frame, padding: 5 * fs,
...
function PackOpening({ pack, cards, ownedBefore, initialPhase = "pack", muted = false, onShare, onSharePack, onDone, onAgain, againLabel = "Abrir outro" }) {
  ...
  useEffect(() => {
    ...
    return () => { clearTimeout(phaseTimer.current); clearTimeout(revealTimer.current); clearTimeout(advanceTimer.current); };
  }, []);
  const onTearDown = (e) => {
  ...
  const flip = () => {
    ...
    if (flipped) {
      ...
      advanceTimer.current = setTimeout(() => {
        setFlipped(false); setAdvancing(false); setFx(null);
        if (idx + 1 >= cards.length) { setPhase("summary"); playFx("summary", muted); }
        else setIdx(idx + 1);
      }, motionReduced ? 0 : 280);
    } else {
      setRevealing(true); setFlipped(true); playFx("flip", muted);
      const delay = motionReduced ? 0 : 520;
      clearTimeout(revealTimer.current);
      revealTimer.current = setTimeout(() => {
        setFx(current.rarity); setRevealing(false);
      ...
      {phase === "pack" || phase === "torn" ? (
        <div style={{ textAlign: "center", animation: phase === "pack" && tearProg === 0 ? "float 2.8s ease-in-out infinite" : "none", position: "relative" }}>
          ...
          <div ref={dragRef} onPointerDown={onTearDown} onPointerMove={onTearMove} onPointerUp={onTearUp} onPointerCancel={onTearUp} onKeyDown={(event) => { if (phase === "pack" && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); tear(); } }} role="button" tabIndex={phase === "pack" ? 0 : -1} aria-label="Abrir pack: arrasta da esquerda para a direita ou prime Enter"
            style={{ width: 250, height: 360, position: "relative", filter: "drop-shadow(0 26px 50px rgba(0,0,0,0.6))", transform: phase === "pack" && tearProg > 0 ? `rotate(${(tearProg - 0.5) * 2.4}deg) scale(${1 + tearProg * 0.012}, ${1 - tearProg * 0.008})` : undefined, animation: phase === "torn" ? "packAway 650ms ease-in 250ms forwards" : "none", touchAction: "none", cursor: phase === "pack" ? "grab" : "default", transition: phase === "pack" ? "transform 45ms linear" : "none" }}>
            {phase === "torn" && (
              <div aria-hidden="true" style={{ position: "absolute", left: 20, right: 20, top: 20, height: 290, zIndex: 0, animation: "cardsEmerge 900ms cubic-bezier(.16,.8,.25,1) forwards", pointerEvents: "none" }}>
                {[2, 1, 0].map((cardIndex) => <div key={cardIndex} style={{ position: "absolute", left: cardIndex * 4, top: cardIndex * 5, transform: `rotate(${(cardIndex - 1) * 3}deg)`, filter: "drop-shadow(0 12px 20px rgba(0,0,0,.45))" }}><CardBack width={230} /></div>)}
              </div>
            )}
        ...
        <div onClick={flip} onKeyDown={(event) => { if (!event.repeat && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); flip(); } }} role="button" tabIndex={0} aria-busy={revealing || advancing} aria-label={revealing ? `A revelar carta ${idx + 1} de ${cards.length}` : advancing ? `A guardar carta ${idx + 1} de ${cards.length}` : flipped ? `Continuar depois de revelar ${current.name}` : `Revelar carta ${idx + 1} de ${cards.length}`} style={{ cursor: revealing || advancing ? "wait" : "pointer", textAlign: "center" }}>
          ...
          <div style={{ perspective: 1100, animation: advancing ? "cardExit 280ms cubic-bezier(.4,0,1,1) forwards" : revealing ? "cardCharge 520ms ease-in-out infinite alternate" : flipped ? "cardSettle 180ms ease-out both" : "cardReady 460ms cubic-bezier(.16,.8,.25,1) both" }} key={idx}>
            <div style={{ position: "relative", width: 260, height: 260 * 1.42, margin: "0 auto", transformStyle: "preserve-3d", transform: revealing ? "translateY(-8px) scale(1.025)" : "translateY(0) scale(1)", transition: revealing ? "transform 520ms cubic-bezier(.2,.7,.3,1.1)" : "transform 180ms ease-out", filter: revealing ? `drop-shadow(0 0 28px ${pack.accent}55)` : "none" }}>
              ...
              <div style={{ position: "absolute", inset: 0, backfaceVisibility: "hidden", WebkitBackfaceVisibility: "hidden", opacity: flipped ? 0 : 1, transform: flipped ? "rotateY(88deg) scale(.96)" : "rotateY(0deg) scale(1)", transition: "transform 260ms ease-in, opacity 100ms 150ms linear", pointerEvents: flipped ? "none" : "auto" }}><CardBack width={260} /></div>
              <div style={{ position: "absolute", inset: 0, backfaceVisibility: "hidden", WebkitBackfaceVisibility: "hidden", opacity: flipped ? 1 : 0, transform: flipped ? "rotateY(0deg) scale(1)" : "rotateY(-88deg) scale(.96)", transition: "transform 320ms 180ms cubic-bezier(.2,.7,.3,1.05), opacity 100ms 180ms linear", pointerEvents: flipped ? "auto" : "none" }}><Card card={current} width={260} showcase={flipped && !revealing} /></div>
            </div>
          </div>
          <div style={{ marginTop: 22, minHeight: 44 }}>
            {flipped && !revealing ? (
              ...
              <div style={{ fontSize: 12, color: revealing ? pack.accent : "#6f87a8", letterSpacing: 2 }}>{revealing ? "A REVELAR…" : `TOCA PARA REVELAR · ${idx + 1}/${cards.length}`}</div>
            )}
          </div>
        </div>
      ) : (
...
function App() {
  ...
  const FASES_ORDEM = [
    {etapa:1,fase:"grupos",grupo:"A"},{etapa:1,fase:"grupos",grupo:"B"},{etapa:1,fase:"grupos",grupo:"C"},
    {etapa:1,fase:"eliminatorias",grupo:null},
    {etapa:2,fase:"grupos",grupo:"A"},{etapa:2,fase:"grupos",grupo:"B"},{etapa:2,fase:"grupos",grupo:"C"},
    {etapa:2,fase:"eliminatorias",grupo:null},
    {etapa:3,fase:"grupos",grupo:"A"},{etapa:3,fase:"grupos",grupo:"B"},{etapa:3,fase:"grupos",grupo:"C"},
    {etapa:3,fase:"eliminatorias",grupo:null},
    {etapa:"finals",fase:"eliminatorias",grupo:null},
  ];

  const getProximaFase = (cfg) => {
    const etapa = cfg.etapa === "finals" ? "finals" : parseInt(cfg.etapa);
    const fase = cfg.fase;
    const grupo = (fase === "grupos" && cfg.grupo) ? cfg.grupo : null;
    const idx = FASES_ORDEM.findIndex((f) => f.etapa === etapa && f.fase === fase && f.grupo === grupo);
    console.log("[getProximaFase] etapa=", etapa, "fase=", fase, "grupo=", grupo, "idx=", idx);
    return idx >= 0 && idx < FASES_ORDEM.length - 1 ? FASES_ORDEM[idx + 1] : null;
  };

  const adminAvancarFase = async () => {
    const proxima = getProximaFase(ligaConfig || {});
    console.log("[adminAvancarFase] proxima=", proxima, "ligaConfig=", ligaConfig, "jaJogou=", jaJogouGrupoAtual);
    if (!proxima) {
      setToast("Já estás na última fase.");
      setTimeout(() => setToast(null), 2000);
      return;
    }
    // só o admin pode avançar, mas tem de ter jogado a fase actual primeiro
    if (!jaJogouGrupoAtual) {
      setToast("Joga primeiro a fase actual antes de avançar.");
      setTimeout(() => setToast(null), 2800);
      return;
    }
    const patch = { etapa: proxima.etapa, fase: proxima.fase, ...(proxima.grupo ? { grupo: proxima.grupo } : { grupo: null }) };
    await adminSaveConfig(patch);
    setLigaConfig((prev) => ({ ...(prev || {}), ...patch, _ts: Date.now() }));
    setToast(`✓ Avançado para ${proxima.etapa === "finals" ? "Finals" : `Etapa ${proxima.etapa}`}${proxima.fase === "grupos" ? ` · Grupo ${proxima.grupo}` : " · Eliminatórias"}`);
  ...
  useEffect(() => {
    ...
    if (cfg.fase === "grupos" && cfg.grupo) {
      ...
      const bracketKey = etapaKey === "finals" ? null : `${etapaKey}_bracket`;

      const tryKeys = bracketKey ? [equipasKey, bracketKey] : [equipasKey];
      const tryLoad = async () => {
  ...
  const resolvePrev = async () => {
    ...
    if (data.prev.resolved) {
      ...
      // modo real: a previsão ficou bloqueada (🔒), o admin pontua depois
      playFx("flip", muted);
    }
  };
  ...
  const shell = (children) => (
    <div style={{ minHeight: "100vh", background: "radial-gradient(140% 90% at 50% 0%, #0C1730 0%, #060A16 60%)", color: "#E7EEF8", fontFamily: "system-ui, sans-serif" }}>
      <style>{`
        ...
        @keyframes packAway { 0% { transform: translate3d(0,0,0) scale(1); opacity: 1; } 45% { opacity: .9; } 100% { transform: translate3d(0,105px,0) scale(.7); opacity: 0; } }
        @keyframes cardsEmerge { 0% { transform: translate3d(0,90px,0) scale(.9); opacity: 0; } 28% { opacity: 1; } 72% { transform: translate3d(0,-38px,0) scale(1.025); opacity: 1; } 100% { transform: translate3d(0,-28px,0) scale(1); opacity: 1; } }
        @keyframes cardReady { 0% { transform: translate3d(0,34px,0) scale(.94) rotate(1.5deg); opacity: .25; } 72% { transform: translate3d(0,-7px,0) scale(1.018) rotate(-.5deg); opacity: 1; } 100% { transform: translate3d(0,0,0) scale(1); opacity: 1; } }
        @keyframes cardExit { 0% { transform: translate3d(0,0,0) scale(1) rotate(0); opacity: 1; } 100% { transform: translate3d(-90px,-18px,0) scale(.82) rotate(-5deg); opacity: 0; } }
        ...
        @keyframes cardSettle { 0% { transform: translate3d(0,-5px,0) scale(1.018); opacity: 1; } 100% { transform: translate3d(0,0,0) scale(1); opacity: 1; } }
        ...
        <main style={{ maxWidth: 1000, margin: "0 auto", padding: "36px 20px 80px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 8, marginTop: 26 }}>
            ...
            {isAdmin && adminProximaFaseAtivo && (
              <button onClick={adminAvancarFase} style={{ ...btn(true), fontSize: 13, background: "#F2C14E22", border: "1px solid #F2C14E66", color: "#F2C14E", padding: "12px 24px" }}>
                ⏭ Próxima Fase — {(() => { const p = getProximaFase(ligaConfig || {}); return p ? `${p.etapa === "finals" ? "Finals" : `Etapa ${p.etapa}`}${p.fase === "grupos" ? ` · Grupo ${p.grupo}` : " · Eliminatórias"}` : "—"; })()}
              </button>
            )}
          </div>
          ...
          <main style={{ maxWidth: 940, margin: "0 auto", padding: "36px 20px 80px" }}>
            ...
            !prev.groups && !prev.groupResult ? (
              <section style={{ marginTop: 26, background: "#0E162E", border: "1px solid #22304d", borderRadius: 16, padding: "30px 20px", textAlign: "center" }}>
                ...
                ) : ligaConfig?.fase === "eliminatorias" ? (
                  <>
                    <div style={{ fontFamily: FONT, fontWeight: 700, fontSize: 16, color: "#F2C14E", marginBottom: 8 }}>
                      {ligaConfig?.etapa === "finals" ? "Finals" : `Etapa ${ligaConfig?.etapa}`} · Eliminatórias
                    </div>
                    <div style={{ fontSize: 13, color: "#8fa3bd", marginBottom: 18 }}>
                      A fase de grupos já terminou. Prevê os vencedores dos quartos, meias-finais e final.
                    </div>
                    <button onClick={drawGroups} style={{ ...btn(true), fontSize: 14, padding: "14px 28px" }}>
                      📋 Carregar bracket das eliminatórias
                    </button>
                  </>
                ) : (
                  <>
            ...
            ) : prev.groupResult?.isFinals || prev.groupResult?.skipGroups ? (
              // Finals ou eliminatórias directas: mostrar bracket directamente
              <>
                {prev.groupResult?.skipGroups && (
                  <section style={{ marginTop: 26, background: "#0E162E", border: "1px solid #F2C14E44", borderRadius: 16, padding: "16px 20px" }}>
                    <div style={{ fontFamily: FONT, fontWeight: 700, fontSize: 13, color: "#F2C14E", letterSpacing: 1 }}>
                      {ligaConfig?.etapa === "finals" ? "🏆 FINALS" : `ETAPA ${ligaConfig?.etapa} · ELIMINATÓRIAS`} · 8 EQUIPAS
                    </div>
                    ...
                    {prev.resolved && (
                      <div style={{ marginTop: 16, background: "#0B1226", border: "1px solid #1BF5A344", borderRadius: 12, padding: "14px 16px" }}>
                        <div style={{ fontFamily: FONT, fontWeight: 700, fontSize: 15, color: "#1BF5A3", marginBottom: 4 }}>
                          Resultado · <span style={{ color: "#F2C14E" }}>+{prev.resolved.score} pts</span>
                        </div>
                        <div style={{ fontSize: 13, color: "#8fa3bd" }}>
                          QF: {prev.resolved.qfHits}/4 · MF: {prev.resolved.sfHits}/2 · Campeão: {prev.resolved.champOk ? "✓" : `✗ (foi ${TEAMS.find(t=>t.id===prev.resolved.champ)?.short ?? prev.resolved.champ})`}
                        </div>
                      </div>
                    )}
                  </section>
              ...
              <>
                ...
                {/* 4 · previsão das eliminatórias — só quando estamos na fase de eliminatórias OU já resolvido OU não é modo real */}
                {prev.bracket && !prev.groupResult?.locked && (ligaConfig?.fase === "eliminatorias" || prev.resolved || ligaConfig?.modo !== "real") && (
                  <section style={{ marginTop: 18, background: "#0E162E", border: "1px solid #22304d", borderRadius: 16, padding: "18px 18px 20px" }}>
                    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 8, marginBottom: 14 }}>
        ...
        return (
          <main style={{ maxWidth: 1380, margin: "0 auto", padding: "36px 20px 80px" }}>
            ...
            <div style={card16}>
              ...
              {/* toggle botão próxima fase na tab competição */}
              <div style={{ marginTop: 14, display: "flex", alignItems: "center", gap: 10 }}>
                <button
                  onClick={() => setAdminProximaFaseAtivo(v => !v)}
                  style={{ ...btn(adminProximaFaseAtivo), fontSize: 12, padding: "8px 16px", border: adminProximaFaseAtivo ? "1px solid #F2C14E66" : "1px solid #22304d" }}>
                  {adminProximaFaseAtivo ? "✓ Botão «Próxima Fase» activo" : "Activar botão «Próxima Fase»"}
                </button>
                {adminProximaFaseAtivo && (
                  <span style={{ fontSize: 11, color: "#6f87a8" }}>
