...
//
// Modo real: lê os resultados das 5 jornadas do grupo do dia em liga_data,
// calcula pontos com base nos resultados reais.
// Cada utilizador só pode jogar cada grupo 1 vez (verificado por etapa+grupo no jHist).
//
...
function scoreRealCard(cardId: string, allMatches: RealMatch[]): ScoreRow {
  ...
  // cartas especiais (edition) têm um campo "ref" que aponta para o id base do jogador
  // (ex: sp-tundi-taca tem ref="tundi", o jogador nos dados é "pl-tundi")
...
Deno.serve(async (req: Request) => {
  ...
  // verificar se já jogou este grupo nesta etapa (modo real)
  if (config.modo === "real" && config.fase === "grupos") {
    const jHist = Array.isArray(state.jHist) ? state.jHist as Record<string, unknown>[] : [];
    const jaJogou = jHist.some((j) => j.etapa === config.etapa && j.grupo === config.grupo && j.modo !== "simulacao_fallback");
